...
static char rcsid[] =
"$Id: asin.c,v 1.4 2003/08/01 04:57:46 ksh Exp $";
...
asin_complex (COMPLEX z)
{
  ...
  w.real = atan2 (z.real, c.real * d.real - c.imag * d.imag);
...
bi_asin (ENTITY *p)
{
  ...
  switch (p->class)
    {
    case scalar:
      return (asin_scalar ((SCALAR *) p));
    case vector:
      return (asin_vector ((VECTOR *) p));
    case matrix:
      return (asin_matrix ((MATRIX *) p));
    default:
      BAD_CLASS (p->class);
      delete_entity (p);
      raise_exception ();
	...
	if (abs (p->v.integer) > 1)
	  {
	    ...
	    s->v.complex = asin_complex (s->v.complex);
	  }
	else
	  {
	    s = (SCALAR *) cast_scalar_integer_real (p);
	    s->v.real = asin (s->v.real);
	  }
	break;
      case real:
	if (fabs (p->v.real) > 1)
...
asin_vector (VECTOR *p)
{
  ...
  switch (p->type)
      ...
      if (neg)
	{
	  ...
	  m = (VECTOR *) apply_vector_complex_complex (asin_complex, p);
	}
      else
	{
	  p = (VECTOR *) cast_vector_integer_real (p);
	  m = (VECTOR *) apply_vector_real_real (asin, p);
	}
      break;
    case real:
      for (i = 0; i < p->nn; i++)
...
asin_matrix (MATRIX *p)
{
  ...
  switch (p->type)
      ...
      if (neg)
	{
	  ...
	  m = (MATRIX *) apply_matrix_complex_complex (asin_complex, p);
	}
      else
	{
	  p = (MATRIX *) cast_matrix_integer_real (p);
	  m = (MATRIX *) apply_matrix_real_real (asin, p);
	}
      break;
    case real:
      for (i = 0; i < p->nn; i++)
